...
### Model (v1.2)
**Purpose:** Control which Claude model executes this task

**Values:**
- `auto` → Automatic selection based on task type (recommended)
- `haiku` → Fast, cheap exploration (read-only tasks)
- `sonnet` → Balanced reasoning (most tasks)
- `opus` → Critical decisions (architecture, security)

**Auto-selection rules:**
- read-only → haiku ($0.25/M tokens)
- security/audit → opus ($15/M tokens)
- architect + user input → opus
- background automation → sonnet ($3/M tokens)
- default → sonnet

**Cost Impact:**
- Haiku: ~$0.02 per small task
- Sonnet: ~$0.15 per medium task
- Opus: ~$0.75 per medium task

**Example:**
```markdown
- **Model:** haiku  # Cheap exploration
- **Model:** opus   # Critical security decision
- **Model:** auto   # Let orchestrator choose (recommended)
```

### Severity (v1.2)
**Purpose:** Priority for task selection and auto-spawn decisions
...
- Critical path tasks → executed first
- High severity → prioritized over medium/low
- Low severity → may be deferred for critical work

**Example:**
```markdown
- **Severity:** critical  # Security fix, blocking bug
- **Severity:** high      # Important feature
- **Severity:** medium    # Standard (default)
- **Severity:** low       # Refactoring, nice-to-have
```

### Resource Constraints (v1.2)

**Purpose:** Override default resource limits for this track

**Defaults (if not specified):**
- Max parallel background: 6
- Max parallel foreground: 2
- Max total parallel: 8
- Cost limit per track: $10.00

**To customize, add to track metadata:**
```yaml
Resource Constraints:
  max_parallel_background: 4
  max_parallel_foreground: 1
  cost_limit_per_track: 5.00
```

**Priority scheduling rules:**
1. Critical path tasks executed first
2. Higher severity tasks prioritized (critical > high > medium > low)
3. Read-only tasks can run in parallel freely
4. Write-shared tasks run sequentially in foreground
...
- Use `/swarm-iosm status` to view real-time progress
